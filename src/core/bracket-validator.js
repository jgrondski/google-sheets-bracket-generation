...
const { isPowerOfTwo, nextPowerOfTwo } = require('../utils/math-utils');
...
class BracketValidator {
  ...
  static getValidationWarnings(config) {
    ...
    if (bracketSize > players.length * 2) {
      warnings.push(`Bracket size (${bracketSize}) is much larger than needed for ${players.length} players`);
    }
